...
export default function Layout() {
  ...
  const carregarNotificacoes = async () => {
    try {
      const res = await api.get('/notificacao/resumo');
      setNaoLidas(res.data.totalNaoLidas);
    } catch {
      // ignora erro (usuário pode não estar logado ainda)
    }
  };
      ...
      <header className="top-header">
        <div className="header-left">
          ...
          <div className="header-brand">
            ...
            <span className="header-brand-text">KODIAK CRM</span>
          </div>
        </div>
      ...
      <div className="layout-body">
        <aside className={`sidebar ${sidebarOpen ? 'sidebar-open' : 'sidebar-closed'}`}>
          <nav className="sidebar-nav">
            ...
            <NavLink to="/clientes">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/><polyline points="16 11 18 13 22 9"/></svg>
              <span>Clientes</span>
            </NavLink>
            <NavLink to="/contatos">
